...
const vertexShader = `
  ...
  uniform float uTime;
  uniform vec2 uResolution; // Used to scale scatter relative to world width
  ...
  void main() {
    ...
    // The geometry is 1x1, but gets scaled by mesh.scale
    // We multiply scatter distance so it flies far in world space
    vec3 scatterState = aCentroid + (aScatterPos * 3.0) + rotatedLocalPos;
    ...
    float rippleX = currentPos.x * uResolution.x * 0.5;
    float rippleY = currentPos.y * uResolution.y * 0.5;
    float ripple = sin(rippleX + uTime * 2.5) * cos(rippleY + uTime * 2.5) * 0.05 * uHover;
    currentPos.z += ripple;

    // Apply exit mechanics (melt down)
    if (uExit > 0.0) {
      float dropAmount = uExit * uExit * 2.0 * (1.0 + fract(aCentroid.x * 13.33));
      currentPos.y -= dropAmount;
      
      float stretch = 1.0 + uExit * 3.0;
...
const fragmentShader = `
  ...
  void main() {
    ...
    // Soften the backface tint (was 0.6): the finale views the corridor from BEHIND,
    // so heavy darkening made the photos read as voids against the sandal fog
    vec3 baseColor = gl_FrontFacing ? texColor.rgb : texColor.rgb * 0.88;
...
interface FloatingShatteredImageProps {
  url: string;
...
export default function FloatingShatteredImage({
  url,
  position,
  rotation = [0, 0, 0],
  width = 4,
  ...
  const [snapTrigger, setSnapTrigger] = useState(0); // time since snapped
  ...
  // Generate 1x1 geometry
  const geometry = useMemo(() => {
    // fracture.ts now generates normalized 1x1 geometry (shards, seed) — the mesh
    // scale [planeW, planeH, 1] provides the size, so no width/height args here.
  ...
  const uniforms = useMemo(
    () => ({
      ...
      uSnap: { value: 0 },
      uExit: { value: 0 },
      uResolution: { value: new THREE.Vector2(planeW, planeH) },
    }),
  ...
  useFrame((_state, delta) => {
    ...
    // FIX (empty tail): this was a SIGNED distance, so once the camera flew past a
    // plane distZ went negative and uExit locked at 1 forever — every photo stayed
    // melted, and because the camera rests at z=-60 looking BACK at the corridor,
    // the last 3-4 scroll sections had no pictures at all. Absolute distance lets
    // shards melt only in a symmetric band at the lens and RE-FORM behind it.
    const distZ = Math.abs(camera.position.z - worldPos.z);
    
    let progress = 0;
    let exit = 0;
    
    // DistZ is distance from camera to image (positive = camera is in front)
    // We want the shards to assemble as the camera gets within 15 units, and lock in at 8 units.
    if (distZ > 15) {
      progress = 0;
    } else if (distZ > 8) {
      progress = 1.0 - ((distZ - 8) / 7);
    } else {
      progress = 1.0;
    }
    ...
    // Trigger snap flash if just assembled
    if (prevProgressRef.current < 1.0 && progress >= 1.0) {
      setSnapTrigger(0.5); // 0.5s flash duration
    }
    prevProgressRef.current = progress;

    if (snapTrigger > 0) {
      setSnapTrigger(Math.max(0, snapTrigger - delta));
    }

    materialRef.current.uniforms.uProgress.value = progress;
    materialRef.current.uniforms.uExit.value = exit;
    materialRef.current.uniforms.uSnap.value = snapTrigger;
